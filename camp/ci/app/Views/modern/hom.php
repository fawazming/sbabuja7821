...
<head>
  ...
  <title>SB Registration</title>
  <script src="https://cdn.tailwindcss.com"></script>
  <style>
    .fade { transition: all 0.4s ease; opacity: 0; transform: translateY(15px); }
    .fade.show { opacity: 1; transform: translateY(0); }
  </style>
</head>
<body class="min-h-screen flex items-center justify-center p-6 bg-gradient-to-br from-green-100 via-white to-green-50">
    ...
    <form id="reg-form" class="fade hidden mt-6 space-y-5">
      ...
      <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
        <div>
          ...
          <input type="email" id="email" name="email" class="w-full border rounded-xl px-4 py-2 focus:ring-2 focus:ring-green-400 focus:border-green-400"/>
        </div>
        <div>
  ...
  <script>
    ...
    async function fetchSBID() {
      ...
      try {
        ...
        const res = await fetch(`/api/fetch-sbid?sbid=${sbid}`);
